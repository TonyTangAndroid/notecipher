...
public class CacheWordService extends Service {
	...
	private final static String TAG = "CacheWordHandlerCacheWordService";
	...
	public int onStartCommand(Intent intent, int flags, int startId) {
		if (intent == null) {
			Log.d(TAG, "onStart: nu n  ll intent");
		...
		if (action.equals(Constants.INTENT_LOCK_CACHEWORD)) {
			if (mSecrets == null) {
				Log.d(TAG, "onStart: LOCK COMMAND received and mSecrets is null..locking");
				lock();
			} else {
				Log.d(TAG, "onStart: LOCK COMMAND received but mSecrets is not null..ignore it");
			}
	...
	public void onDestroy() {
		...
		if (mSecrets != null) {
			...
			mSecrets.destroy();
			mSecrets = null;
			Log.d(TAG, "onDestroy mSecrets to null");
		} else {
	...
	public synchronized void setCachedSecrets(ICachedSecrets secrets) {
		...
		mSecrets = secrets;
		if (secrets == null) {
			Log.d(TAG, "setCachedSecrets() secrets to null");
	...
	public void lock() {
		...
		synchronized (this) {
			if (mSecrets != null) {
				mSecrets.destroy();
				mSecrets = null;
				Log.d(TAG, "lock mSecrets to null");
	...
	private void startTimeout(long seconds) {
		...
		AlarmManager alarmManager = (AlarmManager)getSystemService(ALARM_SERVICE);
		alarmManager.set(AlarmManager.ELAPSED_REALTIME, SystemClock.elapsedRealtime() + (seconds * 1000),
				mTimeoutIntent);
	}
