...
/**
 * This class is designed to accompany any Activity that is interested in the secrets cached by CacheWord. <i>The
 * context provided in the constructor must implement the ICacheWordSubscriber interface.</i> This is so the Activity
 * can be alerted to the state change events.
 */
public class CacheWordHandler {
	private static final String TAG = "CacheWordHandlerCacheWordService";
	...
	/**
	 * Timeout: How long to wait before automatically locking and wiping the secrets after all your activities are no
	 * longer visible This is the default setting, and can be changed at runtime via a preference A value of 0
	 * represents instant timeout A value < 0 represents no timeout (or infinite timeout)
	 */
	static final int DEFAULT_TIMEOUT_SECONDS = 30;

	/**
	 * Tracking service connection state is a bit of a mess. There are three tricky situations:
	 * <ol>
	 * <li>We call bindService() which returns successfully, but we are told to disconnect before the connection
	 * completes (onServiceConnected() is called). This can occur when the activity opens and closes quickly.</li>
	 * <li>2. We shouldn't call unbind() if we didn't bind successfully. Doing so produces Service not registered:
	 * info.guardianproject.cacheword.CacheWordHandler</li>
	 * <li>3. Conversely, We MUST call unbind() if bindService() was called Failing to do so results in Activity FOOBAR
	 * has leaked ServiceConnection We must track the connection state separately from the bound state.</li>
	 * </ol>
	...
	enum ServiceConnectionState {
		CONNECTION_NULL, CONNECTION_INPROGRESS, CONNECTION_CANCELED, CONNECTION_ACTIVE
	}

	enum BindState {
		BIND_NULL, BIND_REQUESTED, BIND_COMPLETED
	...
	/**
	 * Initializes the {@code CacheWordHandler} with a distinct {@link Context} and {@link ICacheWordSubscriber}
	 * objects. Uses default CacheWordSettings
	 */
	...
	/**
	 * Initializes the CacheWordHandler. Use this form when your {@link Context} (e.g, the {@link Activity}) also
	 * implements the {@link ICacheWordSubscriber} interface. The {@code Context} instance passed in MUST implement
	 * {@link ICacheWordSubscriber}, or an {@link IllegalArgumentException} will be thrown at runtime.
	 * <p>
	 * Setting the {@code timeout} to 0 disables the automatic locking timeout, a negative value means use the default
	 * value.
	 * 
	 * @param context
	 *            must implement the {@link ICacheWordSubscriber} interface
	 * @param timeout
	 *            the time in seconds before CacheWord automatically locks
	 */
	...
	/**
	 * Initializes the {@code CacheWordHandler} with distinct {@code Context} and {@link ICacheWordSubscriber} objects.
	 * <p>
	 * Setting the {@code timeout} to 0 disables the automatic locking timeout, a negative value means use the default
	 * value.
	 * 
	 * @param context
	 *            your {@link Application}'s or {@link Activity}'s context
	 * @param subscriber
	 *            the object to notify of CacheWord events
	 * @param timeout
	 *            the time in seconds before CacheWord automatically locks
	 */
	...
	/**
	 * Connect to {@link CacheWordService}, starting it if necessary. Once connected, the attached {@code Context} will
	 * begin receiving CacheWord events. This should be called in your {@link Activity#onResume} or somewhere else
	 * appropriate.
	 */
	public synchronized void connectToService() {
		if (isCacheWordConnected()) {
			Log.d("CacheWordHandler", ":::connectToService:" + " isCacheWordConnected to true");
			return;

		}
		Log.d("CacheWordHandler", ":::connectToService:" + " getBlankServiceIntent");
		Intent cacheWordIntent = getBlankServiceIntent(mContext);
		/*
		 * We start AND bind the service starting - ensures the cacheword service will outlive the activity binding -
		 * allows us to notify the service of active subscribers
		 */
	...
	/**
	 * Detach but don't disconnect from {@link CacheWordService}. CacheWord events will continue to be received, but
	 * this client will not be considered when performing automatic timeouts.
	 */
	public void detach() {
	...
	/**
	 * Use the basic {@link PassphraseSecrets} implementation to derive encryption keys securely. Initializes cacheword
	 * if necessary.
	 * 
	 * @param passphrase
	...
	/**
	 * Changes the passphrase used to encrypt the derived encryption keys. Since the derived encryption key stays the
	 * same, this can safely be called even when the secrets are in use. Only works if you're using the
	 * {@link PassphraseSecrets} implementation. (i.e., Are you using {@link #setPassphrase(char[])} or
	 * {@link #setCachedSecrets(ICachedSecrets)})?
	 * 
	 * @param current_secrets
	 *            the current secrets you're using
	 * @param new_passphrase
	 *            the new passphrase to encrypt the old secrets with
	 * @return null on error or current_secrets on success
	 * @throws {@link IOException}
	 */
	public PassphraseSecrets changePassphrase(PassphraseSecrets current_secrets, char[] new_passphrase)
			throws IOException {
		if (!SecretsManager.isInitialized(mContext)) {
	...
	/**
	 * Request {@link CacheWordService} clear the secrets from memory. This is only a request! The cache should not be
	 * considered wiped and locked until the {@link ICacheWordSubscriber#onCacheWordLocked()} is received.
	 */
	public void lock() {
		if (!isPrepared())
			return;
	 ...
	 * 
	 * @param seconds
	 *            time in seconds to wait before locking, setting to 0 disables the timeout
	...
	/**
	 * Set the {@link Notification} used by {@link CacheWordService} when it runs as a foreground {@link Service}. If
	 * this is set to {@code null}, then {@link CacheWordService} will run as a background {@link Service}.
	 * 
	 * @param notification
	 */
	...
	/**
	 * Create a blank intent to start an instance of {@link CacheWordService}. It is called "blank" because only the
	...
	/**
	 * Get a {@link PendingIntent} that will cause {@link CacheWordService} to lock and wipe the passphrase from memory
	 * once it is sent.
	 * 
	 * @param context
	 * @return
	...
	static public PendingIntent getPasswordLockPendingIntent(Context context) {
		Log.d("CacheWordHandler", ":::getPasswordLockPendingIntent:" + "");
	...
	private void registerBroadcastReceiver() {
		LocalBroadcastManager.getInstance(mContext).registerReceiver(mCacheWordReceiver,
